...
import {roundTo2Decimals} from "@livesession-food-workshop-angular/food-shop/cart/cart-util";


export const getAllOrdersStats = (orders: Order[]): StatsRow => {
  const allExceptCanceled = orders.filter(
    (ord) => ord.orderStatus !== OrderStatus.ORDER_CANCELED
  );
  return {
...
export const getDeliveredOrdersStats = (orders: Order[]): StatsRow => {
  const deliveredOrders = orders.filter(
    (ord) => ord.orderStatus === OrderStatus.ORDER_DELIVERED
  );
  return {
...
export const getCanceledOrdersStats = (orders: Order[]): StatsRow => {
  const cancledOrders = orders.filter(
    (ord) => ord.orderStatus === OrderStatus.ORDER_CANCELED
  );
  return {
